...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val grouped = columnNames.groupBy(col =>
      ...
      else if (working.exists(col.startsWith)) "working"
      else if (other.exists(col.startsWith)) "other"
      else "NA"
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when(df("teage").between(15, 22), "young")
      .when(df("teage").between(23, 55), "active").otherwise("elder").as("age")

    // Create columns that sum columns of the initial dataset
    // Hint: you want to create a complex column expression that sums other columns
    //       by using the `+` operator between them
    // Hint: don’t forget to convert the value to hours
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce(_ + _) / 60 as ("primaryNeedsProj")
    val workProjection: Column = workColumns.reduce(_ + _) / 60 as ("workProj")
    val otherProjection: Column = otherColumns.reduce(_ + _) / 60 as ("otherProj")
    df
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed
      ...
      .agg(round(avg("primaryNeedsProj"), 1).as("primaryNeeds"),
        round(avg("workProj"), 1).as("work"),
        round(avg("otherProj"), 1).as("other"))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    ...
    select working, sex, age
          ,round(avg(primaryNeedsProj), 1) primaryNeeds
          ,round(avg(workProj), 1) work
          ,round(avg(otherProj), 1) other
    from $viewName
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(row => TimeUsageRow(
      ...
      row.getAs[Double]("primaryNeedsProj"),
      row.getAs[Double]("workProj"),
      row.getAs[Double]("otherProj")
    ))
